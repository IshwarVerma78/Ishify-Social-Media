...
//                 {
//                     mode == "text" && (
//                         <textarea className='bg-transparent text-white w-full h-full p-6 text-lg resize-none focus:outline-none' placeholder='Whats on you mind?'  onChange={(e)=>setText(e.target.value)} value= {text} />
//                     )
//                 }
...
//                         ) : (
//                             <video src={previewUrl} className='object-contain max0h-full' />
//                         )
//                     )
//                 }
...
//                 <label className={`flex-1 flex items-center justify-center gap-2 rounded cursor-pointer ${mode === 'media' ? "bg-white text-black": "bg-zinc-800"}`} >
...
const StoryModal = ({ setShowModal, fetchStories }) => {
  ...
  const handleCreateStory = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || error.message || "Failed to create story"
      );
    }
  };
